    ...
    <div class="list_wrap" v-loading="loading">
      ...
      <el-form label-position="right" label-width="120px" :model="businessdata">
        <el-form-item label="测试地址">
          <el-input autofocus="true" v-model="businessdata.TTestUrl"></el-input>
        </el-form-item>
        <div class="seletitem">
          <span>申请入口</span>
          <el-radio v-for="(entry,index) in entrys" :key="index" v-model="businessdata.MApplyEntryType" :label="entry.RealValue">{{entry.ShowName}}</el-radio>
        </div>
        <div class="seletitem">
          <span>全行通用</span>
          <el-switch
        ...
        <el-form-item label="说明原因" v-if="!businessdata.MIsAllowAllBank">
          <el-input v-model="businessdata.MNotAllowInfo"></el-input>
        </el-form-item>

        <div class="seletitem">
          <span>调用接口</span>
          <el-checkbox-group v-model="businessdata.MCallInterface">
            <el-checkbox v-for="(item,index) in checkForm" :key="index" :label="item.ShowName" size="medium"></el-checkbox>
          </el-checkbox-group>
        </div>
        <el-form-item label="分类标签">
          <el-input v-model="businessdata.MTags" placeholder="选填"></el-input>
        </el-form-item>
      </el-form>
      <el-row>
  ...
  export default {
    ...
    methods: {
      submitBusiness() {
        var vm = this
        var CurUser = JSON.parse(localStorage.getItem("CurUser"));
        this.addBusinessData.MTestUrl = this.businessdata.TTestUrl
        this.addBusinessData.MApplyEntryType = this.businessdata.MApplyEntryType
        if (this.businessdata.MIsAllowAllBank) {
          this.addBusinessData.MIsAllowAllBank = 1
        } else {
          this.addBusinessData.MIsAllowAllBank = 0
          this.addBusinessData.MNotAllowInfo = this.businessdata.MNotAllowInfo
        }
        this.addBusinessData.MTags = this.businessdata.MTags
        //新状态值要改变
