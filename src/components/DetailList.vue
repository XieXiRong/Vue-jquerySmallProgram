<template>
  <div class="detail_content">
    ...
    <div class="businessname" v-if="showSrManagerInfo.CurrStateNo>=21">
      ...
      <el-input v-if="!showDetail" value="showSrManagerInfo.MApplyEntryType" size="small"
                v-model="showSrManagerInfo.MApplyEntryType"></el-input>
    </div>
    ...
    <div class="businessname" v-if="showSrManagerInfo.CurrStateNo>24">
      <p>测试商户号</p>
    ...
    <div class="businessname" v-if="showSrManagerInfo.CurrStateNo>=31">
      <p>附件</p>
      <p class="fujian"><a v-for="(item,index) in downloadName" :key="index" :href="User.baseUrl+downloadUrl[index]" :download="item">{{item}}</a></p>
    </div>
    <div class="businessname" v-if="showSrManagerInfo.CurrStateNo>=41">
      <p>测试用例</p>
      <p class="fujian"><a v-for="(item1,index) in testCasesName" :key="index" :href="User.baseUrl+testCases[index]" :download="item1">{{item1}}</a></p>
  ...
  export default {
    ...
    mounted() {
      ...
      $.ajax({
        ...
        success: function (result) {
          ...
          for (var j = 0; j < entrys.length; j++) {
            if (vm.showSrManagerInfo.MApplyEntryType == entrys[j].RealValue) {
      ...
      if(CurUser.ROLETYPE==0){
        if(this.showSrManagerInfo.CurrStateNo>=54){
            this.showBusiness1 = true
        }
      }else if(CurUser.ROLETYPE==1){
        if(this.showSrManagerInfo.CurrStateNo>24){
          this.showBusiness1 = true
        }
      }
